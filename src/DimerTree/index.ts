	...
	const tag: TagContract = {
		...
		compile(parser, buffer, token) {
			...
			buffer.outputExpression(
				'template.renderWithState(...ctx.renderer.getComponentFor(node))',
				token.filename,
				token.loc.start.line,
				false
			)
		...
		/**
		 * Sharing renderer with the context to pull components for different
		 * node
		 */
		run(ctx) {
			ctx.getter('renderer', () => renderer, true)
		},
	}
